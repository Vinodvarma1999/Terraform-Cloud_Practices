...
locals {
  ...
  formatted_number = format("%03d", local.last_used_number + var.instance_count)
}
...
resource "aws_instance" "my-project" {
  ...
  tags = {
    Name = "${var.location}${var.app_name}${var.env}${var.service_name}${local.formatted_number}${count.index}"
  }
} 
